export const post = {
  title: "China in 18 Days: From Beijing to Hong Kong",
  description: "A comprehensive guide to exploring China's highlights, from ancient wonders to modern cities",
  author: "Local Expert",
  publishDate: "2024-03-25",
  modifiedDate: "2024-03-25",
  image: "https://images.unsplash.com/photo-1508804185872-d7badad00f7d?auto=format&fit=crop&q=80",
  content: `
# China in 18 Days: The Ultimate First-Timer's Guide

China is a land of contrasts where ancient traditions meet rapid modernization. This 18-day itinerary takes you through the country's most iconic destinations, from the ancient walls of Beijing to the stunning karst landscapes of Yangshuo, ending in the vibrant metropolis of Hong Kong.

## Beijing (Days 1-3)
### Imperial Capital & Great Wall

The journey begins in Beijing, China's political and cultural heart. The first two days are dedicated to exploring the city's imperial heritage:

- **Forbidden City**: Start early to explore this massive palace complex
- **Temple of Heaven**: Visit in the morning to watch locals practicing tai chi
- **Summer Palace**: Perfect for afternoon strolls through imperial gardens
- **Hutongs**: Explore these ancient alleyways for authentic local life

Day 3 is dedicated to the Great Wall at Mutianyu section:
- Less crowded than Badaling
- Beautiful mountain scenery
- Cable car available for easy access
- Toboggan ride down (optional but fun!)

## Xi'an (Days 4-5)
### Terra Cotta Warriors & Muslim Quarter

The ancient capital of Xi'an offers:
- **Terracotta Warriors**: Plan a full morning here
- **City Walls**: Bike rental available for cycling the complete circuit
- **Muslim Quarter**: Evening food tour recommended
- **Big Wild Goose Pagoda**: Beautiful at sunset

## Chengdu (Days 6-7)
### Pandas & Spicy Food

Chengdu, capital of Sichuan province, famous for:
- **Giant Panda Breeding Research Base**: Visit early morning
- **People's Park**: Experience local tea culture
- **Jinli Street**: Traditional architecture and street food
- **Sichuan Opera**: Evening face-changing performance

## Leshan & Emei Shan (Days 8-10)
### Buddhist Mountain Retreat

- **Leshan Giant Buddha**: World's largest stone Buddha
- **Mount Emei**: Sacred Buddhist mountain
  - Stay in a monastery
  - Watch sunrise from the Golden Summit
  - Take the cable car for spectacular views

## Yangtze River Cruise (Days 11-14)
### Three Gorges Journey

Three days cruising through:
- **Qutang Gorge**: Most dramatic scenery
- **Wu Gorge**: Known for its mysterious beauty
- **Xiling Gorge**: Longest of the three
- Shore excursions to:
  - Three Gorges Dam
  - Ghost City of Fengdu
  - Lesser Three Gorges

## Yangshuo (Days 15-17)
### Karst Mountains & River Life

The highlight of Guangxi province:
- **Li River cruise**: Stunning karst landscape
- **Bicycle through villages**: Rice paddies and local life
- **Rock climbing**: World-class locations
- **Cooking classes**: Learn local specialties
- **Light show**: Impression Liu Sanjie performance

## Hong Kong (Day 18)
### Modern City Finale

End your journey in vibrant Hong Kong:
- **Victoria Peak**: City views
- **Star Ferry crossing**: Harbor experience
- **Temple Street Night Market**: Shopping and street food

## Best Time to Visit

- **Spring (March-May)**: Comfortable temperatures, beautiful blooms
- **Autumn (September-November)**: Clear skies, perfect for photography
- Avoid summer months (June-August) due to heat and crowds
- Winter can be cold in the north but pleasant in the south

## Travel Tips

1. **Visa Requirements**: Most visitors need a visa - apply well in advance
2. **Transportation**: 
   - High-speed trains are excellent for city-to-city travel
   - Internal flights save time for longer distances
   - Download DiDi app (Chinese Uber) for city transport
3. **Internet**: 
   - Get a VPN before arrival
   - Download offline maps
4. **Language**: 
   - Learn basic Mandarin phrases
   - Have your hotel's address in Chinese characters
5. **Payment**: 
   - WeChat Pay and Alipay are ubiquitous
   - Carry some cash for smaller vendors

Remember to respect local customs, dress modestly at religious sites, and always carry your passport for hotel check-ins.
`,
  country: "China",
  flag: "🇨🇳",
  locations: [
    {
      name: "Forbidden City",
      googleMapLink: "https://maps.google.com/?q=Forbidden+City,+Beijing",
      contentSections: {
        introduction: "The Forbidden City was the Chinese imperial palace from the Ming Dynasty to the end of the Qing Dynasty. It is located in the center of Beijing and now houses the Palace Museum.",
        bestTimeToVisit: "Early morning (8:30-10:00) to avoid crowds. Closed on Mondays.",
        mapEmbed: `<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3056.8874264848656!2d116.39020661744384!3d39.916345!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x35f052967800f319%3A0x5a29c2158e9c5bb7!2sForbidden%20City!5e0!3m2!1sen!2sus!4v1647850455252!5m2!1sen!2sus" width="100%" height="450" style="border:0;" allowfullscreen="" loading="lazy"></iframe>`,
        customInfo: [
          "Purchase tickets in advance during peak season",
          "Audio guides available in multiple languages",
          "Plan 3-4 hours for a thorough visit"
        ]
      }
    }
  ],
  callToAction: {
    position: 2,
    text: "Planning a trip to China? Save all these locations to your personal map!",
    buttonText: "Download Locator",
    link: "https://locator.ltd"
  },
  tags: ["China", "Travel Guide", "Beijing", "Xi'an", "Chengdu", "Yangshuo", "Hong Kong"],
  category: "Travel Guide",
  readingTime: "15 minutes"
};