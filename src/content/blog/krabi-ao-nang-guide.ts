export const post = {
  title: "Krabi & Ao Nang Guide: Honeymoon Edition",
  description: "🌴✨ Join us on our mini honeymoon adventure in Thailand! From emotional moments at Railay Beach to stunning resorts, discover our personal guide to this paradise.",
  publishDate: "2024-03-15",
  modifiedDate: "2024-03-15",
  author: "Locator Team",
  image: "https://upload.wikimedia.org/wikipedia/commons/c/ca/Ao_Nang_beach_panorama_1.jpg",
  country: "thailand",
  flag: "🇹🇭",
  locations: [
    {
      name: "The Irish Embassy Pub",
      googleMapLink: "https://maps.google.com/maps?q=The+Irish+Embassy+Pub+Ao+Nang",
      contentSections: {
        introduction: "I don't know what it is about Thailand and Irish pubs, but they just work. Great food, great vibes, no notes. 🍻🇮🇪",
        bestTimeToVisit: "Evening for dinner and drinks",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.2776460563787!2d98.82340731744384!3d8.036861999999999!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6c3c0f291e1%3A0x89c24377c7b5ee9d!2sThe%20Irish%20Embassy%20Pub!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Live sports screenings",
          "International menu",
          "Great atmosphere",
          "Air conditioned"
        ]
      }
    },
    {
      name: "Starbucks Ao Nang",
      googleMapLink: "https://maps.google.com/maps?q=Starbucks+Ao+Nang",
      contentSections: {
        introduction: "You know Starbucks, but the interior here is 🔥 and perfect for a quick pit stop when you need a break. ☕✨",
        bestTimeToVisit: "Morning or afternoon for a coffee break",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.2776460563787!2d98.82340731744384!3d8.036861999999999!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6c3c0f291e1%3A0x89c24377c7b5ee9d!2sStarbucks%20Ao%20Nang!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Beautiful interior design",
          "Air conditioned",
          "Good WiFi",
          "Perfect for a break from the heat"
        ]
      }
    },
    {
      name: "Centara Grand Beach Resort & Villas Krabi",
      googleMapLink: "https://maps.google.com/maps?q=Centara+Grand+Beach+Resort+%26+Villas+Krabi",
      contentSections: {
        introduction: "The best hotel on Krabi Beach—luxury meets incredible views. Can't go wrong here. 🌅🏨",
        bestTimeToVisit: "Any time - the resort is beautiful year-round",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.7123456789012!2d98.83456789012345!3d8.012345678901234!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6b4567890ab%3A0x1234567890abcdef!2sCentara%20Grand%20Beach%20Resort%20%26%20Villas%20Krabi!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Private beach access",
          "Multiple restaurants",
          "Luxury spa",
          "Stunning ocean views"
        ]
      }
    },
    {
      name: "Rayavadee Resort",
      googleMapLink: "https://maps.google.com/maps?q=Rayavadee+Resort+Krabi",
      contentSections: {
        introduction: "Hands down THE most luxurious place to stay in the area. Remote, peaceful, and absolutely stunning. 🛶💎",
        bestTimeToVisit: "November to April (dry season)",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.8901234567890!2d98.83456789012345!3d8.023456789012345!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6b4567890ab%3A0x1234567890abcdef!2sRayavadee!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Access to three beaches",
          "World-class dining",
          "Unique pavilion-style accommodation",
          "Surrounded by nature"
        ]
      }
    },
    {
      name: "Railay Beach",
      googleMapLink: "https://maps.google.com/maps?q=Railay+Beach+Krabi",
      contentSections: {
        introduction: "Swimming in the rain, surrounded by cliffs and mist, was an emotional experience. Yes, I cried (don't tell my wife). OH - there's a super good smoothie shop there called Summer Fresh! 😅⛰️🌧️",
        bestTimeToVisit: "Early morning or late afternoon to avoid crowds",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3956.9012345678901!2d98.83567890123456!3d8.034567890123456!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6b5678901cd%3A0x2345678901abcdef!2sRailay%20Beach!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Accessible only by boat",
          "Famous for rock climbing",
          "Summer Fresh smoothie shop",
          "Beautiful sunset views"
        ]
      }
    },
    {
      name: "Khaothong Hill",
      googleMapLink: "https://maps.google.com/maps?q=Khaothong+Hill+Viewpoint+Krabi",
      contentSections: {
        introduction: "It's a bit of a drive, but the views? Absolutely worth it. Perfect way to end a day. 🚗✨",
        bestTimeToVisit: "Sunset for the best views",
        mapEmbed: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3957.0123456789012!2d98.83678901234567!3d8.045678901234567!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3051f6b6789012de%3A0x3456789012abcdef!2sKhaothong%20Hill!5e0!3m2!1sen!2sth!4v1647331799273",
        customInfo: [
          "Panoramic viewpoint",
          "Great for photography",
          "Sunset viewing spot",
          "Worth the drive"
        ]
      }
    }
  ],
  callToAction: {
    position: 4,
    text: "Ready to explore Krabi?",
    buttonText: "Save These Locations",
    link: "https://locator.ltd"
  }
};
